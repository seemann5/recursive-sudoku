...
class SudokuProblem:
    ...
    def __init__(self, order):
        ...
        # For pretty output
        self.color_list = ["\033[0m", "\033[91m", "\033[92m", "\033[94m"]
    ...
    def __str__(self) -> str:
        return f"{self.color_list[2]}----- Inflation:{self.color_list[0]} {self.order} sources, " \
                + f"{self.n_alices} Alices, " \
                + f"{self.n_cycles} triangle-isomorphic subgraphs " \
                + self.color_list[2] + "-----" + self.color_list[0]

    def color_outcome(self, outcome: int) -> str:
        return self.color_list[outcome] + str(outcome) + self.color_list[0]
            ...
            for j in range(self.sp.order):
                ...
                if i == j:
                    ...
                    if self.filling[self.sp.ij_to_k[(i,j)]]:
                        ret += f"{self.sp.color_list[2]}F{self.sp.color_list[0]}"
                    else:
                        ret += "."
            ret += "\n"
        return ret
...
class FillingStep:
    ...
    def variable_str(self, lim: int = None) -> str:
        ...
        None: everything"""
        ret = f"{self.sp.color_list[2]}Update{self.sp.color_list[0]} " \
            + f"{self.sp.k_to_str[self.initial_k]}"
        ...
        for r, (k1,k2,k3) in enumerate(self.check_rules):
            if r == 0:
                ...
                ret += "\n"
            ret += f"  {self.sp.color_list[1]}check{self.sp.color_list[0]} cycle" \
            ...
            for j in range(self.sp.order):
                ...
                if i == j:
                    ...
                    if outcome == 0:
                        ...
                        ret += self.sp.color_outcome(outcome)
            ret += "\n"
        return ret

    def sparse_str(self) -> str:
        ret = "\n"
        for i in range(self.sp.order):
            ret += "  "
            if i > 0:
...
class SudokuSolver:
    ...
    def set_initial_outcomes_and_complete(
        ...
        if verb or show_initial_grid:
            print("The initial grid is now setup to be")
            print(base_grid)
            wprint("About to try to complete!")

        # Now, let's try to complete the grid.

        # Data
        self.grid_states = [
            GridState(self.sp) 
                for i in range(len(self.final_filling_steps) + 1)
            ]
        self.grid_states[0].copy_from(base_grid)

        completable = self.recursive_fill_grid(verb, 0)

        return completable

    def recursive_fill_grid(
            self,
            verb: bool = False,
            current_depth: int = 0
            ) -> bool:
        ...
        # Assume well-initialized
        cur_grid = self.grid_states[current_depth]

        if verb:
            print(f"\n------- Entering recursion at depth {current_depth} --------")

        # Assume not well-initialized
        child_grid = self.grid_states[current_depth+1]

        cur_fill_step = self.final_filling_steps[current_depth]
        ...
        for outcome in (1,2,3):
            ...
            consistent_so_far = \
                child_grid.update_based_on(cur_fill_step, outcome)

            if verb:
                print(f"Child grid after updating",
                    f"{self.sp.k_to_str[cur_fill_step.initial_k]}",
                    f"with outcome",
                    self.sp.color_outcome(outcome),":")
                print(child_grid)

            if not consistent_so_far:
                if verb:
                    input("Not consistent! Moving on...")
                continue
            
            # Else, it's consistent. If full we're good: 
            if current_depth == len(self.final_filling_steps)-1:
                if verb:
                    print("Consistent and full - hence complete!")
                return True, child_grid

            # Try to fill in the next one
            # if the grid isn't full...
            if current_depth < len(self.final_filling_steps)-1:
                if verb:
                    input("Consistent but not full, about to go recursive...")
                completable, filled_grid = \
                    self.recursive_fill_grid(verb,current_depth + 1)

                if completable:
                    return True, filled_grid
